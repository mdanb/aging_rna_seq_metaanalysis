...
def pca(gene_exp_datasets, experiments, age, labels, subplots_dims, figsize, savefile, by_experiment=True):
    ...
    for idx, (gene_expression_data, experiments, age, labels) in enumerate(zip(gene_exp_datasets, experiments, age,
                                                                               labels)):
        ...
        if (by_experiment):
            ...
            def generate_colormap(number_of_distinct_colors: int = 80):
                ...
                number_of_distinct_colors_with_multiply_of_shades = \
                    int(math.ceil(number_of_distinct_colors / number_of_shades) * number_of_shades)

                # Create an array with uniformly drawn floats taken from <0, 1) partition
                linearly_distributed_nums = np.arange(number_of_distinct_colors_with_multiply_of_shades) / \
                                            number_of_distinct_colors_with_multiply_of_shades

                # We are going to reorganise monotonically growing numbers in such way that there will be
                # single array with saw-like pattern but each saw tooth is slightly higher than the one before
                # First divide linearly_distributed_nums into number_of_shades sub-arrays containing
                # linearly distributed numbers
                arr_by_shade_rows = linearly_distributed_nums.reshape(number_of_shades,
                                                                      number_of_distinct_colors_with_multiply_of_shades
                                                                      // number_of_shades)

                # Transpose the above matrix (columns become rows) - as a result each row contains saw
                # tooth with values slightly higher than row above
                ...
                # HSV colour map is cyclic (https://matplotlib.org/tutorials/colors/colormaps.html#cyclic),
                ...
                for i in range(3):
                    for j in range(upper_partitions_half):
                        modifier = np.ones(number_of_shades) - initial_cm[lower_half + j * number_of_shades:
                                                                          lower_half + (j + 1) * number_of_shades, i]
                        modifier = j * modifier / upper_partitions_half
                        initial_cm[lower_half + j * number_of_shades:
                                   lower_half + (j + 1) * number_of_shades, i] += modifier

                return ListedColormap(initial_cm)

            data_with_experiments = gene_expression_data.merge(experiments, left_index=True,
            ...
            if len(gene_exp_datasets) > 1:
                if (sp_dim1 > 1):
                    ax[int(idx / sp_dim1)][idx % sp_dim2].set_prop_cycle(color=[scalarMap.to_rgba(i)
            ...
            for _, group in data_with_experiments.groupby('Bioproject'):
                if len(gene_exp_datasets) > 1:
                    if (sp_dim1 > 1):
                        ax[int(idx / sp_dim1)][idx % sp_dim2].scatter(df_2d.loc[group.index][0],
                                                                      df_2d.loc[group.index][1], s=150)
                    else:
            ...
            while (ages):
                for name, group in age_groups:
                    if(name == ages[0]):
                        if (len(gene_exp_datasets) > 1):
                            if (sp_dim1 > 1):
                                ax[int(idx / sp_dim1)][idx % sp_dim2].plot(group[group['longevity'] == 0].dim1,
                                                                           group[group['longevity'] == 0].dim2,
                                                                            label='long', marker='o', linestyle='',
                                                                           markersize=12, c=colors[color_index])
                                ax[int(idx / sp_dim1)][idx % sp_dim2].plot(group[group['longevity'] == 1].dim1,
                                                                           group[group['longevity'] == 1].dim2,
                                                                            label='normal', marker='*', linestyle='',
                                                                           markersize=12, c=colors[color_index])
                                ax[int(idx / sp_dim1)][idx % sp_dim2].plot(group[group['longevity'] == 2].dim1,
                                                                           group[group['longevity'] == 2].dim2,
...
create_similarity_matrix([getmm_gene_expression_all_data], "getmm_all_data_similarity_matrix", (1,1), (10,10))
...
pca([getmm_gene_expression_no_outliers], [experiments], [age], [labels], (1,1), (15,15),
    "pca_getmm_no_outliers_by_experiment", True)

pca([getmm_gene_expression_no_outliers], [experiments], [age], [labels], (1,1), (15,15), "pca_getmm_no_outliers_by_age",
    False)

create_similarity_matrix([getmm_gene_expression_no_outliers], "getmm_no_outliers_similarity_matrix", (1,1), (10,10))

# GeTMM & Experiment Combat Seq
combat_seq_getmm_ge_no_outliers_and_singles = pd.read_csv(
    '../common_datastore/getmm_combat_seq_no_outliers_and_singles_gene_expression.csv')
combat_seq_getmm_ge_no_outliers_and_singles.columns = ['Sample'] + \
                                                      list(combat_seq_getmm_ge_no_outliers_and_singles.columns[1:])
...
create_similarity_matrix([combat_seq_getmm_ge_no_outliers_and_singles],
                         "combat_seq_getmm_no_outliers_and_singles_similarity_matrix", (1,1), (10,10))

pca([combat_seq_getmm_ge_no_outliers_and_singles], [experiments], [age], [labels],
    (1,1), (15,15), "pca_by_age_combat_seq_getmm_no_singles_and_outliers", False)
...
age_adjusted_all_no_outliers_ged = pd.read_csv(
    '../common_datastore/combat_seq_age_corrected_getmm_gene_expression_no_outliers.csv')
...
pca([age_adjusted_all_no_outliers_ged], [experiments], [age], [labels], (1,1), (15,15),
    "combat_seq_age_corrected_all_getmm_no_outliers_pca_by_age", False)

# Filtered Data
# day 1 and older
day_1_and_older = pd.read_csv(
    '../common_datastore/combat_seq_experiment_corrected_day_1_and_older_getmm_gene_expression_no_outliers.csv')
...
# L4 and younger
L4_and_younger = pd.read_csv(
    '../common_datastore/combat_seq_experiment_corrected_L4_and_younger_getmm_gene_expression_no_outliers.csv')
...
combat_seq_double_getmm_ge_no_outliers_and_singles = pd.read_csv(
    '../common_datastore/combat_seq_experiment_and_age_corrected_getmm_gene_expression_no_outliers.csv')
combat_seq_double_getmm_ge_no_outliers_and_singles.columns = ['Sample'] + \
                                                    list(combat_seq_double_getmm_ge_no_outliers_and_singles.columns[1:])
combat_seq_double_getmm_ge_no_outliers_and_singles = \
    combat_seq_double_getmm_ge_no_outliers_and_singles.set_index('Sample')
...
# GO filtered
combat_seq_getmm_GO_filtered_gene_expression_no_singles = \
    pd.read_csv('../common_datastore/combat_seq_getmm_GO_filtered_gene_expression_no_singles_and_outliers.csv')
combat_seq_getmm_GO_filtered_gene_expression_no_singles.columns = \
    ['Sample'] + list(combat_seq_getmm_GO_filtered_gene_expression_no_singles.columns[1:])
combat_seq_getmm_GO_filtered_gene_expression_no_singles = \
    combat_seq_getmm_GO_filtered_gene_expression_no_singles.set_index('Sample')

create_similarity_matrix([combat_seq_getmm_GO_filtered_gene_expression_no_singles],
                         "combat_seq_getmm_GO_filtered_gene_expression_no_singles_similarity_matrix", (1,1), (10,10))
