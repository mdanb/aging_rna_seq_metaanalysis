...
class SAUCIE(object):
    ...
    def __init__(self, input_dim,
        ...
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, input_dim], name='x')
        self.y = tf.compat.v1.placeholder(tf.float32, shape=[None, input_dim], name='y')
        self.batches = tf.compat.v1.placeholder(tf.int32, shape=[None], name='batches')
        self.is_training = tf.compat.v1.placeholder(tf.bool, shape=[], name='is_training')
        self.learning_rate_tensor = tf.compat.v1.placeholder(tf.float32, shape=[], name='learning_rate_tensor')
    ...
    def _restore(self, restore_folder):
        ...
        self.saver = tf.compat.v1.train.import_meta_graph('{}.meta'.format(ckpt.model_checkpoint_path))
    ...
    def _build_layers(self):
        ...
        if self.lambda_b:
            h1 = tf.compat.v1.layers.dense(self.x, self.layers[0], activation=lrelu, name='encoder0', use_bias=True)

            h2 = tf.compat.v1.layers.dense(h1, self.layers[1], activation=lrelu, name='encoder1', use_bias=True)

            h3 = tf.compat.v1.layers.dense(h2, self.layers[2], activation=lrelu, name='encoder2', use_bias=True)

            self.embedded = tf.compat.v1.layers.dense(h3, 2, activation=tf.identity, name='embedding', use_bias=True)
            self.embedded = nameop(self.embedded, 'embeddings')

            h5 = tf.compat.v1.layers.dense(self.embedded, self.layers[2], activation=lrelu, name='decoder0', use_bias=True)

            h6 = tf.compat.v1.layers.dense(h5, self.layers[1], activation=lrelu, name='decoder1', use_bias=True)

            h7 = tf.compat.v1.layers.dense(h6, self.layers[0], activation=lrelu, name='decoder2', use_bias=True)
            h7 = nameop(h7, 'layer_c')

            self.reconstructed = tf.compat.v1.layers.dense(h7, self.input_dim, activation=tf.identity, name='recon', use_bias=True)
            ...
            self.embedded = tf.compat.v1.layers.dense(h3, self.layers[3], activation=tf.identity, name='embedding', use_bias=True)
            self.embedded = nameop(self.embedded, 'embeddings')
            ...
            h7 = tf.compat.v1.layers.dense(h6, self.layers[0], activation=tf.nn.relu, name='decoder2', use_bias=True)
            h7 = nameop(h7, 'layer_c')

            self.reconstructed = tf.compat.v1.layers.dense(h7, self.input_dim, activation=tf.identity, name='recon', use_bias=True)
            self.reconstructed = nameop(self.reconstructed, 'output')
        else:
            h1 = tf.compat.v1.layers.dense(self.x, self.layers[0], activation=lrelu, name='encoder0')

            h2 = tf.compat.v1.layers.dense(h1, self.layers[1], activation=tf.nn.sigmoid, name='encoder1')

            h3 = tf.compat.v1.layers.dense(h2, self.layers[2], activation=lrelu, name='encoder2')

            self.embedded = tf.compat.v1.layers.dense(h3, self.layers[3], activation=tf.identity, name='embedding')
            self.embedded = nameop(self.embedded, 'embeddings')

            h5 = tf.compat.v1.layers.dense(self.embedded, self.layers[2], activation=lrelu, name='decoder0')

            h6 = tf.compat.v1.layers.dense(h5, self.layers[1], activation=lrelu, name='decoder1')

            h7 = tf.compat.v1.layers.dense(h6, self.layers[0], activation=lrelu, name='decoder2')
            h7 = nameop(h7, 'layer_c')


            self.reconstructed = tf.compat.v1.layers.dense(h7, self.input_dim, activation=tf.identity, name='recon')
            self.reconstructed = nameop(self.reconstructed, 'output')
    ...
    def _build_losses(self):
        ...
        if self.lambda_b:
            with tf.compat.v1.variable_scope('reconstruction_mmd'):
                self._build_reconstruction_loss_mmd(self.reconstructed, self.x)
            with tf.compat.v1.variable_scope('batchcorrection'):
                self._build_reg_b()

        else:
            with tf.compat.v1.variable_scope('reconstruction'):
                self._build_reconstruction_loss(self.reconstructed, self.x)

        if self.lambda_c:
            with tf.compat.v1.variable_scope('clustering'):
                self.loss_c = 0

                act = tbn('layer_c:0')
                act = act / tf.reduce_max(input_tensor=act)
                self._build_reg_c(act)

        if self.lambda_d:
            with tf.compat.v1.variable_scope('intracluster_distances'):
    ...
    def _build_reconstruction_loss(self, reconstructed, y):
        ...
        """
        self.loss_recon = tf.reduce_mean(input_tensor=(self.reconstructed - y)**2)

        self.loss_recon = nameop(self.loss_recon, 'loss_recon')
        tf.compat.v1.add_to_collection('losses', self.loss_recon)
    ...
    def _build_reconstruction_loss_mmd(self, reconstructed, y):
        ...
        :param y: the tensorflow op for the target
        """
        refrecon = tf.boolean_mask(tensor=reconstructed, mask=tf.equal(self.batches, 0))
        refy = tf.boolean_mask(tensor=y, mask=tf.equal(self.batches, 0))
        l = (refy - refrecon)**2
        self.loss_recon += tf.reduce_mean(input_tensor=l)


        nonrefrecon = tf.boolean_mask(tensor=reconstructed, mask=tf.equal(self.batches, 1))
        nonrefy = tf.boolean_mask(tensor=y, mask=tf.equal(self.batches, 1))

        mean1, var1 = tf.nn.moments(x=nonrefrecon, axes=0)
        mean2, var2 = tf.nn.moments(x=nonrefy, axes=0)
        l = ( ((nonrefrecon - mean1) / (tf.sqrt(var1+1e-6)+1e-6)) - ((nonrefy - mean2) / (tf.sqrt(var2+1e-6)+1e-6)) )**2

        self.loss_recon += .01*tf.reduce_mean(input_tensor=l)
    ...
    def _build_reg_d(self, act):
        ...
        same_cluster = same_cluster - tf.reduce_min(input_tensor=same_cluster)
        same_cluster = same_cluster / tf.reduce_max(input_tensor=same_cluster)

        dists = self._pairwise_dists(self.x, self.x)
        dists = tf.sqrt(dists + 1e-3)

        intracluster_distances = dists * same_cluster
        intracluster_distances = tf.reduce_mean(input_tensor=intracluster_distances)
    ...
    def _build_reg_c(self, act):
        ...
        # sum down neurons
        p = tf.reduce_sum(input_tensor=act, axis=0, keepdims=True)
        # normalize neuron sums
        normalized = p / tf.reduce_sum(input_tensor=p)

        self.loss_c += self.lambda_c * tf.reduce_sum(input_tensor=-normalized * tf.math.log(normalized + 1e-9))

        self.loss_c = nameop(self.loss_c, 'loss_c')
        tf.compat.v1.add_to_collection('losses', self.loss_c)
    ...
    def _build_reg_b(self):
        ...
        e = self.embedded / tf.reduce_mean(input_tensor=self.embedded)
        K = self._pairwise_dists(e, e)
        K = K / tf.reduce_max(input_tensor=K)
        K = self._gaussian_kernel_matrix(K)

        # reference batch
        i = 0
        batch1_rows = tf.boolean_mask(tensor=K, mask=tf.equal(self.batches, i))
        batch1_rowscols = tf.boolean_mask(tensor=tf.transpose(a=batch1_rows), mask=tf.equal(self.batches, i))

        K_b1 = batch1_rowscols
        n_rows_b1 = tf.cast(tf.reduce_sum(input_tensor=tf.boolean_mask(tensor=tf.ones_like(self.batches), mask=tf.equal(self.batches, i))), tf.float32)
        K_b1 = tf.reduce_sum(input_tensor=K_b1) / (n_rows_b1**2)
        ...
        j = 1
        batch2_rows = tf.boolean_mask(tensor=K, mask=tf.equal(self.batches, j))
        batch2_rowscols = tf.boolean_mask(tensor=tf.transpose(a=batch2_rows), mask=tf.equal(self.batches, j))

        K_b2 = batch2_rowscols
        n_rows_b2 = tf.cast(tf.reduce_sum(input_tensor=tf.boolean_mask(tensor=tf.ones_like(self.batches), mask=tf.equal(self.batches, j))), tf.float32)
        K_b2 = tf.reduce_sum(input_tensor=K_b2) / (n_rows_b2**2)

        var_within[j] = K_b2
        batch_sizes[j] = n_rows_b2

        K_12 = tf.boolean_mask(tensor=K, mask=tf.equal(self.batches, i))
        K_12 = tf.boolean_mask(tensor=tf.transpose(a=K_12), mask=tf.equal(self.batches, j))
        K_12_ = tf.reduce_sum(input_tensor=tf.transpose(a=K_12))
        ...
        tf.compat.v1.add_to_collection('losses', self.loss_b)

    def _build_total_loss(self):
        """Collect all of the losses together."""
        self.loss = 0
        for l in tf.compat.v1.get_collection('losses'):
            self.loss += l
    ...
    def _gaussian_kernel_matrix(self, dist):
        ...
        return tf.reshape(tf.reduce_sum(input_tensor=tf.exp(-s), axis=0), tf.shape(input=dist)) / len(sigmas)

    def _pairwise_dists(self, x1, x2):
        """Helper function to calculate pairwise distances between tensors x1 and x2."""
        r1 = tf.reduce_sum(input_tensor=x1 * x1, axis=1, keepdims=True)
        r2 = tf.reduce_sum(input_tensor=x2 * x2, axis=1, keepdims=True)

        D = r1 - 2 * tf.matmul(x1, tf.transpose(a=x2)) + tf.transpose(a=r2)
    ...
    def get_loss(self, load, batch_size=256):
        ...
        for i, batch in enumerate(load.iter_batches(batch_size=batch_size)):
            ...
            batch_losses = self.sess.run(tf.compat.v1.get_collection('losses'), feed_dict=feed)
