...
while read NEWFILE; do
	...
	if [[ $NEWFILE =~ _1 ]]; then
		...
		{ set -e ; kallisto quant -i \ 
		Caenorhabditis_elegans.WBcel235.cdna.all.index --plaintext -o \
		$file_dir -t 12 $(ls $file_dir/*gz -1 | head -1) \
		$(ls $file_dir/*gz -1 | tail -1) ; \
		rm $file_dir/*gz ;  } &
	else
		{ set -e ; kallisto quant -i \
		Caenorhabditis_elegans.WBcel235.cdna.all.index --plaintext -o \ 
		$file_dir --single -t 12 -l 250 -s 30 $(ls $file_dir/*gz) ; \
		rm $file_dir/*gz ;  } &
	fi
done
